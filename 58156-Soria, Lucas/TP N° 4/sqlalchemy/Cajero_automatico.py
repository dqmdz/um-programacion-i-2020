...
class CajeroVacioError(Exception):
    pass
...
class Cajero_automatico:
    ...
    def extraer_dinero(self, monto):
        ...
        if monto < 0:
            raise ValorError("Error. No se puede extraer cantidades de " +
                             "dinero negativas")
        if dispo == [0, 0, 0, 0]:
            raise CajeroVacioError("Error. No se puede extraer dinero si" +
                                   " el cajero esta vacio")
        if monto > total:
            raise CantidadError("Error. No se puede extraer mas dinero del" +
                                " que hay disponible en el cajero")
        if monto % 100 != 0:
            raise MultiplicidadError("Error. No se puede extraer dinero que" +
            ...
            while billetes[x] > 0:
                if dispo[x] > 0:
                    ...
                    if x != 3:
                        ...
                        raise BilleteError("Error. No hay una combinación" +
            ...
            while billetes[x] > 0 and suma <= sacar:
                if dispo[x] > 0:
                    ...
                    if x < 3:
                        ...
                        raise BilleteError("Error. No hay una combinación de" +
                                           " billetes que nos permita " +
                                           "extraer ese monto")
                    billetes[x] = 0
