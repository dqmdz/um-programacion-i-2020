...
class Cajero_automatico:
    ...
    def extraer_dinero(self, monto):
        ...
        if monto < 0:
            raise ValorError("Error. No se puede extraer cantidades de dinero negativas")
        if dispo == [0, 0, 0, 0]:
            raise CajeroVacioError("Error. No se puede extraer dinero si el cajero esta vacio")
        if monto > total:
            raise CantidadError("Error. No se puede extraer mas dinero del que hay disponible en el cajero")
        if monto % 100 != 0:
            raise MultiplicidadError("Error. No se puede extraer dinero que no sea múltiplo de 100")
