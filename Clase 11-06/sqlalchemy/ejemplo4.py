...
class DBCon:
    def __init__(self):
        ...
        str_conexion = 'mysql+pymysql://fernando:fernando@'
        str_conexion += 'mysql:3306/sqlalchemy'
        self.engine = create_engine(str_conexion, echo=True)

        # Equipo local con MySQL
        # str_conexion = 'mysql+pymysql://fernando:fernando@
        # 192.168.10.106:3306/sqlalchemy'
...
class Persona(Base):
    ...
    def __repr__(self):
        ...
        for reg in self.registros:
            resultado = resultado + "{}\n".format(reg)
        salida = 'Persona {}: apellido, nombre: {}, {}. '\
            .format(self.id, self.apellido, self.nombre)
        salida += 'Lista de registros:\n {}'\
            .format(resultado)
        return salida
